...
if __name__ == '__main__':
    ...
    # cam = Camera(640, 480, 57, 3, [1.5, 0, 0.65], [0,0,0.3], [-1, 0, 0], 0.105, 10)
    ...
    if True:
        ...
        # board = aruco.GridBoard_create(col, row, 0.09, 0.02, aruco_dict, start_id)
    ...
    # chess_board_len = 0.08

    # chess_board_len = 0.06
    ...
    for y in range(y_step):
        for z in range(z_step):
            ...
            # q = Rotation.from_rotvec([0, 0, 0]).as_quat()
            ...
            for k in range(rot_step):
                ...
                # q = (Rotation.from_rotvec([-np.pi/18 + k * np.pi/18, -np.pi/18 + k * np.pi/18, 0]) * Rotation.from_quat([0, 0, 1, 0])).as_quat()

                # more_rot = Rotation.from_rotvec([-np.pi/18 + k * np.pi/18, -np.pi/18 + k * np.pi/18, 0])

                rv = np.random.rand()
                if rv > 0.5:
                    # more_rot = Rotation.from_rotvec([-angle_step * 2 + k * angle_step, angle_step * 2 - k * angle_step, -angle_step_2 * 2 + k * angle_step_2])
                    more_rot = Rotation.from_rotvec([-angle_start + k * angle_gap, -angle_start + k * angle_gap, -angle_start + k * angle_gap])
                else:
                    # more_rot = Rotation.from_rotvec([-angle_step * 2 + k * angle_step, angle_step * 2 - k * angle_step, angle_step_2 * 2 - k * angle_step_2])
                    more_rot = Rotation.from_rotvec([-angle_start + k * angle_gap, -angle_start + k * angle_gap, angle_start - k * angle_gap])
                # more_rot = Rotation.from_rotvec([0, 0, 0])
                _x, _y, _z, _w = (more_rot * Rotation.from_rotvec([0, np.pi/2, 0])).as_quat()
                q = [_w,_x,_y,_z]

                bot.move_to([ x, -0.2 + y * 0.4/y_step, 0.55 - z * 0.3/z_step ], q)
                # bot.move_to([ 0.3, -0.3, 0.5 ], q)
    ...
    # p_b2t_g = p_b2t_g[frame_ids[:,0]]
    # p_c2w_e = p_c2w_e[frame_ids[:,0]]
